...
export default class Board {
  constructor(game, _x = 0, _y = 0) {
    ...
    this.swapSound = game.sound.add('swap')
    this.submitSound = game.sound.add('submit')
    this.submitFailedSound = game.sound.add('submit_failed')
    const style = {
      boundsAlignH: 'center',
      boundsAlignV: 'center',
      font: '18pt Arial',
    ...
    for (let x = _x; x < NUM_COLUMNS + _x; x++) {
      for (let y = _y; y < NUM_ROWS + _y; y++) {
        this.tiles.create(x, y, 'tiles')
        if (y >= NUM_ROWS - 2) {
          this.overlays.create(x, y, 'overlay')
        }
        if (y === NUM_ROWS - 2 && x % 2 === 0) {
          this.highlights.create(x, y, 'highlight')
        }
      }
    }
  }
  ...
  clearHighlights(matches) {
    this.overlays.getChildren().forEach(child => child.hide())
    this.highlights.getChildren().forEach(child => child.hide())
  }
  ...
  applyHighlights(matches) {
    ...
    matches.forEach(match => {
      ...
      let indexes = [2, 4, 6]
      let highlight = this.highlights.getChildren()[Math.floor(pos / 2)]
      highlight.show()
      highlight.setFrame(indexes.indexOf(size))
  ...
  _getTileById(id, thing = 'tiles') {
    return this[thing].getChildren().find(tile => tile.id === id)
  }
          ...
          for (let j = 0; j < 4; j++) {
            if (i !== j) {
              ...
              if (arraysEqual(shapeVariation, frames)) {
                ...
                const spawnIndex =
                  frames[0] === 2 ? 1 : frames[0] === 3 ? 0 : frames[0]
  ...
  _getRectOfTiles(x, size) {
    ...
    let tiles = []
    for (
      let y = secondLastRow + x;
      y < NUM_COLUMNS * NUM_ROWS;
      y += NUM_COLUMNS
    ) {
